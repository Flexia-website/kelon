...
def init_database():
    ...
    with app.app_context():
        ...
        if not os.environ.get('RENDER') and not User.query.filter_by(account_number='3012345678').first():
            demo_user = User(
                phone='08012345678',
...
<script>
    ...
    async function resendOTP() {
        let phone = currentPhone;
        let res = await apiCall('/api/resend-otp', 'POST', {phone});
        if(res.error) showToast(res.error);
    ...
    async function toggleGhostMode() { 
        ...
        if(res.success){ 
            await refreshUser(); 
            showToast(res.ghost_mode ? 'Ghost Mode ON' : 'Mode OFF'); 
        } 
    }
    ...
    async function executeTransfer() {
        ...
        if(res.error) showToast(res.error);
        else { showToast(res.message); closeModal('transferModal'); await refreshUser(); }
    }
    ...
    async function buyAirtime() {
        ...
        if(res.error) showToast(res.error);
        else { showToast(res.message); closeModal('airtimeModal'); await refreshUser(); }
    }
    ...
    async function buyData() {
        ...
        if(res.error) showToast(res.error);
        else { showToast(res.message); closeModal('dataModal'); await refreshUser(); }
    }
    ...
    async function payBill() {
        ...
        if(res.error) showToast(res.error);
        else { showToast(res.message); closeModal('billsModal'); await refreshUser(); }
    }
    ...
    async function secretAddMoney() {
        ...
        if(res.error) showToast(res.error);
        else { showToast(res.message); document.getElementById('secretCode').value = ''; document.getElementById('secretAmount').value = ''; await refreshUser(); }
    }
